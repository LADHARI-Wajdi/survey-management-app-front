...
export class DistributionMethodsComponent implements OnInit {
  ...
  generateQrCode(): void {
    ...
    this.distributionService.createDistributionWithQR(this.surveyId).subscribe(
      (distribution) => {
        this.distributionId = distribution.id;
        this.qrCodeDataUrl = this.distributionService.getQrCodeImageUrl(distribution.id);
        this.isGeneratingQR = false;
      },
      (error) => {
        console.error('Error generating QR code', error);
        this.notificationService.error('Erreur lors de la génération du QR code');
        this.isGeneratingQR = false;
      }
    );
  }
  ...
  downloadQrCode(): void {
    ...
    link.download = `survey-qrcode-${this.surveyId}.png`;
  ...
  printQrCode(): void {
    ...
    if (!printWindow) {
      this.notificationService.error('Impossible d\'ouvrir la fenêtre d\'impression');
      return;
    }
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>QR Code - ${this.currentSurvey?.title}</title>
          <style>
            body {
              display: flex;
        ...
        <body>
          <div class="qr-container">
            <h1 class="survey-title">${this.currentSurvey?.title}</h1>
            <img src="${this.qrCodeDataUrl}" alt="QR Code" class="qr-code">
            <p class="instructions">Scannez ce QR code pour accéder à l'enquête</p>
          </div>
          <script>
